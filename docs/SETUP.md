...
9. `DOCUMENT_RETENTION_DAYS` הוא אופציונלי וברירת המחדל היא 2920 ימים: כל צילום נמחק אוטומטית מ־Firestore ומ־Storage כעבור שמונה שנים מיום העלאתו, בלי שחזור. שמונה ולא שבע, מפני ששבע השנים בתקנות ניהול פנקסי חשבונות נספרות מתום שנת המס, כך שחשבונית מתחילת השנה צריכה להישמר כמעט שמונה שנים מיום הצילום. אפשר לקצר (למשל `365` לשנה), להאריך עד `3650`, או `0` כדי לבטל את המחיקה האוטומטית. אין צורך להגדיר כלום כדי לקבל את ברירת המחדל. עלות האחסון בהיקף של חנות היא שברי שקלים בחודש גם בתקופה מלאה. האפליקציה מציגה את התקופה שהשרת מדווח, כך שהמסך מתעדכן מעצמו אחרי שינוי.
10. ודא את המשאבים הקיימים: min=0, max=2, concurrency=2, memory=512 MiB, CPU=1, timeout=60 שניות, request-based billing.
11. לפני Deploy: מספר שאינו בפורמט E.164 מונע מה־revision לעלות. יש לתקן את הערך במסך Variables & Secrets, בלי לפרסם אותו. גם `DOCUMENT_RETENTION_DAYS` שאינו מספר שלם בין 0 ל־3650 מונע מה־revision לעלות.
...
11. צלם **חשבונית אחת אמיתית** ומלא את חמשת הפרטים מהנייר. בדוק בסיכום כל סכום ובמיוחד את המע״מ מול המסמך. ודא שלפני לחיצה על ״אשר ושמור חשבונית״ היא אינה מופיעה ברשימת החשבוניות.
12. נסה גם חשבונית ללא מע״מ: בחר ״אין מע״מ · 0%״ וודא שהסכום לפני מע״מ שווה לסכום הכולל.
...
16. פתח את צילומי חשבונית הבדיקה, לחץ ״מחק צילום לגמרי״ ואשר. הצילום צריך להיעלם מהמסך, ולחיצה על ״פתח עמוד / קובץ״ לאותו קובץ לא אמורה להיות זמינה יותר. מתחת לצילומים צריכה להופיע תקופת השמירה שהוגדרה בשרת. זו מחיקה אמיתית ללא שחזור, ולכן יש לבצע אותה על צילום בדיקה בלבד.
17. הורד גיבוי JSON ו־CSV מסונן לחודש ובדוק את הרשומות. מחק את חשבוניות הבדיקה דרך הממשק עם אישור; אפשר להפוך את ספק הבדיקה ללא פעיל.
