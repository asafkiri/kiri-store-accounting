...
const draft = (fields) => ({ fields: { deductions: [], documentType: "invoice", ...fields } });
...
test("the arithmetic question still fires on a document that does not reconcile", () => {
  ...
  assert.ok(invoiceQuestions(off).includes("arithmetic"));
  const exact = draft({
    total: "1635.50",
    vat: "249.48",
    subtotal: "1386.00",
  ...
  assert.equal(
    invoiceQuestions(exact).includes("arithmetic"),
    false,
  );
});
