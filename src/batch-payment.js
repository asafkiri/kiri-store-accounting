...
import { escapeHtml as e, money, displayDate, today, methods } from "./format.js";
...
export async function batchPaymentForm(ctx, supplierId) {
  ...
  const root = ctx.dialog("תשלום לכמה חשבוניות", `<form class="batch-payment-form">
    ...
    <div class="batch-invoice-list">${draft.candidates.map(i => `<label class="batch-invoice"><input type="checkbox" name="invoiceSelection" value="${e(i.id)}" ${f.selectedIds.includes(i.id) ? "checked" : ""}><span><strong>${e(displayDate(i.invoiceDate))}</strong>${i.documentType === "credit" ? '<small>חשבונית זיכוי</small>' : ""}</span><strong>${e(money(i.finalAgorot))}</strong></label>`).join("")}</div>
